...
class irrep_delta():
    def __init__(self, prefix, nk1, nk2, nk3, Ef, ne):
        ...
        self.Lk=len(np.unique(self.Vkkdata[:,3]))
        self.Libnd=len(np.unique(self.Vkkdata[:,8]))

        #parse k points index
        ...
        #get input data of this code from input file
    ...
    def GetElStructure(self, bxsffile, N, Ef):
        ...
        #calculate the number of k_points, band number, minus of electron energy and Ef
        NN=N-1
    ...
    def GetDelta(self, ne, Lk, Libnd):
        eevvv = np.loadtxt('delta'+'/'+'dkkt'+str(ne)+'.txt').view(complex).reshape(-1)
        ...
        for i in range(Lk):
            for iii in range(int(Libnd/2)):
                ...
                dkk0[i][2*iii+0, 2*iii+1] = -dkk0[i][2*iii+0, 2*iii+1]
                dkk0[i][2*iii+1, 2*iii+0] = -dkk0[i][2*iii+1, 2*iii+0]
        return dkk0
    ...
    def GetWfc(self, Lk, nbwan):
        ...
        for i in range(LLwfc):
            ...
            ibnd=int(d1[i,1]-1)
            jbnd=int(d1[i,2]-1)
            wfck[ibnd,ik,jbnd]=d1[i,3]+1j*d1[i,4]
        return wfck
                ...
                for nky in range(NN):
                    for nkz in range(NN):
                        ...
                        if dkkxyz[i,nkx,nky,nkz]>0.00001 or dkkxyz[i,nkx,nky,nkz]<-0.00001:
                            if self.spin_xyz[i,nkx,nky,nkz][0]<0:
    ...
    def SaveToFrmsf(self, bxsffile, nk1, nk2, nk3, NN, nbwan, prefix, ne, Ekkxyz, dkkxyz):
        ...
        bxsf=open(bxsffile)
        lead=bxsf.readlines()
        f=open(prefix+"t"+str(ne)+'.frmsf','w')
        ii=0
    ...
    def CheckCha(self, operation, dkkxyz, nbwan, NN, Lk, kindex, lambdaFS, Ekkxyz):
        avg_character = 0
        #the k position of lambdaFS file generated by epw-gkkgkk.x are writed in crystal axis
        #the ratation of k vector follows the crystal transmission matrix, the rotation of spin vector follows the cartisan transmission matrix.
        if operation == "inversion":
            trans_matrix_k = [[-1, 0, 0], [0, -1, 0], [0, 0, -1]]
            trans_matrix_spin = [[1, 0, 0], [0, 1, 0], [0, 0, 1]]
        if operation == "C2x":
            trans_matrix_k = [[0, -1, 1], [0, -1, 0], [1, -1, 0]]
            trans_matrix_spin = [[1, 0, 0], [0, -1, 0], [0, 0, -1]]
        for ibnd in range(nbwan):
            for i in range(Lk):
                ...
                nkz=int(kz*NN+10**-6)
                k_vector = [kx, ky, kz]
                if abs(dkkxyz[ibnd, nkx, nky, nkz]) > 0.001 and ibnd%2==0: 
                    #calculate the k index after rotation
                    nkx_trans, nky_trans, nkz_trans = self.TransKvec(trans_matrix_k, k_vector, NN)
                    s_0 = self.spin_xyz[ibnd, nkx, nky, nkz]
                    #the spin expection of g(\psi_k)
                    g_s_k = np.dot(trans_matrix_spin, s_0)
        ...
        plt.ylim([-2,2])
        print(operation+" : "+str(avg_character / Lk))
    ...
    def RunAll(self):
        ...
        #reformat the eigenvector in the full k-mesh and rotate the triplet eigenvector to have the T_0 components only [c.f. eqs.(12) and (16) in the paper]
        ...
        #check the symmetry character of inversion operation
        self.CheckCha("inversion", dkkxyz, self.nbwan, self.NN, self.Lk, self.kindex, self.lambdaFS, self.Ekkxyz)
        
        #check the symmetry character of C2x operation
        self.CheckCha("C2x", dkkxyz, self.nbwan, self.NN, self.Lk, self.kindex, self.lambdaFS, self.Ekkxyz)
        #note: for other system, different symmetry can be checked by different operation

        #delta fuction of /delta only finite when abs(Ek-Ef)<fthick, we need interpolate delta function to full k space for plot the delta function
        dkkxyz = self.InterpolateDelta(self.nk1, self.nk2, self.nk3, self.nbwan, self.NN, dkkxyz)
    
        
        #dump eigenvector in .frmsf format for plot figure
        self.SaveToFrmsf(self.bxsffile, self.nk1, self.nk2, self.nk3, self.NN, self.nbwan, self.prefix, self.ne, self.Ekkxyz, dkkxyz)


#irrep_delta(prefix, nk1, nk2, nk3, efermi, ne)
#Calculate the eigenvector corresponding to the nth smallest eigenvalue
#ne = 0 means leading even parity channel;ne = 1~3 is not physical meaningful;ne = 4 means leading odd parity channel
irrep = irrep_delta("pb", 20, 20, 20, 11.2876, 3)
irrep.RunAll()